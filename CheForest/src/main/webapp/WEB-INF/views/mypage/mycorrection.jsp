...
<form id="addForm" name="addForm" method="post" enctype="multipart/form-data" action="/member/update">
			...
			<c:choose>
				...
				<c:otherwise>
					...
					<div class="form-group">
						...
						<input type="password" id="confirmPassword" name="confirmPassword" class="form-control" placeholder="비밀번호 재입력" minlength="6">
					</div>
				</c:otherwise>
			</c:choose>
...
<script>
	...
	$(document).ready(function () {
		...
		$("#nickname").on("blur", function () {
			...
			fetch("/member/nicknameCheck?nickname=" + encodeURIComponent(nickname))
					.then(res => res.json())
					.then(result => {
						if (result.available) {
